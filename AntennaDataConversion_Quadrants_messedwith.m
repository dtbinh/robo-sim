...
%% RECTANGULAR COORDINATES TO SPHERICAL
    ...
    [azm,elm,rm] = cart2sph(X,Y,Z);
    for i=1:size(X)
    r(i) = sqrt(X(i)^2+Y(i)^2+Z(i)^2);
    
    %Determine elevation angle by E-Plane quadrant    
        if X(i)>=0 && Y(i)>=0       %Quadrant1
        el(i) = (180/pi)*(atan(sqrt(X(i)^2+Y(i)^2)/Z(i))) + 90;
        elseif X(i)<0 && Y(i)>=0    %Quadrant2
        el(i) = (180/pi)*(atan(sqrt(X(i)^2+Y(i)^2)/Z(i))) + 270;    
        elseif X(i)<0 && Y(i)<0     %Quadrant3
        el(i) = (180/pi)*(atan(sqrt(X(i)^2+Y(i)^2)/Z(i))) + 270;  
        else                        %Quadrant4
        el(i) = (180/pi)*(atan(sqrt(X(i)^2+Y(i)^2)/Z(i))) + 90;      
        end
    %Determine azimuth angle by H-Plane quadrant    
        if X(i)>=0 && Z(i)>=0       %Quadrant1
        az(i) = (180/pi)*atan(Y(i)/X(i)) + 90;
        elseif X(i)<0 && Z(i)>=0    %Quadrant2
        az(i) = (180/pi)*atan(Y(i)/X(i)) + 270;    
        elseif X(i)<0 && Z(i)<0     %Quadrant3
        az(i) = (180/pi)*atan(Y(i)/X(i)) + 270;  
        else                        %Quadrant4
        az(i) = (180/pi)*atan(Y(i)/X(i)) + 90;    
        end    
    ...
    %Develop a uniform grid in 10 degree steps for az and el
    azs = 0;%*pi/180;
    azi = 10;%*pi/180;
    aze = 350;%*pi/180;
    els = 0;%*pi/180;
    ...
    Zm = griddata(azm,elm,rm,Az_m,El_m,'cubic');
      %xlswrite('data2.xlsx', Zi, 'Sheet3','B2');
