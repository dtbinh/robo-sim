...
% Rectangular Data
 ...
 % 3D points of the surface
    ...
    %% Figure2 is used purely for reference/diagnosis of Figure1, Plot2
    figure(2)                   
    subplot(1,2,1)              % Figure2, Plot1
    plot(Xp,Yp)                 %image to be rotated to generate Figure1, Plot2
    phi = -pi:2*pi:pi;          %single 3D representation of plot(Xp,Yp)
    X = repmat(Xp,size(phi));   %X coordinate
    Y = Yp * cos(phi);          %Y coordinate
    Z = Yp * sin(phi);          %Z coordinate
    subplot(1,2,2)              % Figure2, Plot2
